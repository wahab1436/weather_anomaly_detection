...
def display_data_status():
    ...
    for file_name, file_path in files_to_check:
        if os.path.exists(file_path):
            ...
            if time_diff < 24:
                status_class = "data-success"
                status_text = f"✓ {file_time.strftime('%Y-%m-%d %H:%M')}"
            elif time_diff < 72:
                status_class = "data-warning"
                status_text = f"⚠ {file_time.strftime('%Y-%m-%d %H:%M')}"
            else:
            ...
            <div class="data-status {status_class}">
                <strong>{file_name}:</strong> {status_text}
            </div>
            """, unsafe_allow_html=True)
        else:
            st.sidebar.markdown(f"""
            <div class="data-status data-error">
                <strong>{file_name}:</strong> ✗ Not found
            </div>
            """, unsafe_allow_html=True)
...
def main():
    ...
    with tab1:
        ...
        # Recent data table
        if not data['daily_stats'].empty:
            st.markdown('<h3 class="sub-header">Recent Data</h3>', unsafe_allow_html=True)
    ...
    <div style="text-align: center; color: #6B7280; font-size: 0.875rem;">
        ...
        <p>Data Source: National Weather Service (weather.gov) | Last Update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        <p>Professional Weather Monitoring System | All Rights Reserved</p>
    </div>
    """, unsafe_allow_html=True)
