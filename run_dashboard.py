...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('logs/run_dashboard.log'),
    ...
    for step_name, step_func in steps:
        ...
        try:
            ...
            if success:
                ...
                # Ask if we should continue (only in CLI mode)
                if step_name != "Web Scraping" and not sys.argv[0].endswith('streamlit'):
...
def schedule_scraping():
    ...
    try:
        from src.scraping.scrape_weather_alerts import schedule_scraping as schedule_job
        schedule_job()
    except KeyboardInterrupt:
...
if __name__ == "__main__":
    # Only run CLI when executed directly, not when imported by Streamlit
